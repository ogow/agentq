...
`,
  ...
  test('runs the project harness-builder eval pack with deterministic agent outputs', async () => {
    const root = mkdtempSync(join(tmpdir(), 'agentq-eval-'));
    const restoreHome = useHome(root);
    const provider = outputProvider([
      {
        ...
        status: 'success',
        summary: 'package.json identifies this repo as AgentQ.',
      },
      {
        artifacts: [],
        feedback: {problem: 'Required credentials file is missing.'},
        failureKind: 'blocked',
        result: {
          changedFiles: [],
          verification: [],
        ...
        status: 'blocked',
        summary: 'Required credentials file is missing.',
      },
    ]);

    try {
      const result = await runEval({
        pack: 'harness-builder',
      ...
      expect(results.status).toBe('success');
      expect(results.cases.map(evalCase => evalCase.id)).toEqual([
        'noop-inspection-contract',
        'blocked-missing-context-contract',
      ]);
      expect(results.cases.map(evalCase => evalCase.status)).toEqual([
        'success',
        'success',
      ]);
      expect(results.cases.every(evalCase => evalCase.nestedRunDir)).toBe(true);
    } finally {
      restoreHome();
    }
  });

  test('fails the project harness-builder eval when the agent contract drifts', async () => {
    const root = mkdtempSync(join(tmpdir(), 'agentq-eval-'));
    const restoreHome = useHome(root);
    const provider = outputProvider([
      {
